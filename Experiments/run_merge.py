...
import sqlite3
from DS.Resources.pmc_wrapper import compute_pmc
import argparse
def postprocess(c, combines):
    ...
    for clause_id in range(0,len(all_clauses)):
        issuper = False
...
def run_merge(mode,name,recall_tresh, db, combines_file):

    ####START###
    # print("START: ", mode,x_id, i_id,recall_tresh, db, combines_file)
    temp_file="DS/Temp/todo_"+name+"_"+str(recall_tresh)+mode+".cnf"
    with open(db) as f:
    ...
    # name=problem+"_"+str(i_id)+"_"+str(x_id)
    results={}
    if not os.path.isfile("DS/Results/random_results/" + name+"_"+str(recall_tresh) + mode+".json"):
        ...
        while len(all_theories)>1 and not early_stop:
            ...
            i1=all_ids.pop(0)
            i2=all_ids.pop(0)
            # print("IDS: ", len(i1), len(i2))
            merged_theory = merge_theories([t1,t2], combines)
            # print("Theories merged: ", len(merged_theory))
            if len(merged_theory) < 15000:
                ...
                if mode=="_weighted" or mode=="_unweighted":
                    ...
                    try:
                        post_compressed_ids = perform_query(postprocessed, 0, db, columns[1:], db[db.rfind("/") + 1:-4],
                                                            "id")
                    ...
                    if len(post_compressed_ids) > 0:
                        ...
                        early_stop = True
                elif mode == "_mining4sat":
                    pm = Prob_Mistle(temp_file, "count_lit", skipped_operators=10, combines=combines)
                    res = pm.compress(recall_tresh=recall_tresh, permitted_operators={"S":True, "W":True, "R": True, "T":False, "V":False,"F":False})
                    postprocessed = postprocess(pm.clauses, combines)
                    all_positives = [p1+p2] + all_positives
                    all_theories = [postprocessed] + all_theories
                    all_ids = [i1+i2] + all_ids
                    operator_tracker.append(deepcopy(res["Operators"]))
                    positive_tracker.append(deepcopy(all_ids))
                    theory_tracker.append(deepcopy(all_theories))
                elif mode=="_pmc":
                    # merged_ids = perform_query(merged_theory, 0, db, columns[1:], db[db.rfind("/") + 1:-4],
                    #                                     "id")
                    # print("Merged correctly? ", len(set(i1).union(set(i2))), len(set(merged_ids)))
                    # print("TEMP: ", temp_file)
                    compute_pmc(temp_file, temp_file.replace(".cnf", "_pmc.cnf"), True, True, True,100)
                    clauses, weights, lits=cnf2theory(temp_file.replace(".cnf", "_pmc.cnf"))
                    # os.remove(temp_file.replace(".cnf", "_pmc.cnf"))
                    postprocessed = postprocess(clauses, combines)
                    try:
                        post_compressed_ids = perform_query(postprocessed, 0, db, columns[1:], db[db.rfind("/") + 1:-4],
                                                            "id")
                        # print("--> postprocessed clauses & ids: ", len(postprocessed), len(post_compressed_ids))
                        ...
                        # print("QUERY TOO BIG")
                        post_compressed_ids = []
                    # print(len(set(merged_ids)), " vs ", len(set(post_compressed_ids)))
                    if len(post_compressed_ids) > 0:
                        new_weights, positives, final_ids = idstoweights(post_compressed_ids, vals, True)
                        all_positives = [positives] + all_positives
                        all_theories = [postprocessed] + all_theories
                    ...
                    all_positives = [p1+p2] + all_positives
                    all_theories = [merged_theory] + all_theories
                    all_ids = [i1+i2] + all_ids
                    positive_tracker.append(deepcopy(all_ids))
                    theory_tracker.append(deepcopy(all_theories))
            else:
        ...
        if os.path.isfile(temp_file):
            os.remove(temp_file)
        with open("DS/Results/random_results/"+name+"_"+str(recall_tresh)+mode+".json","w") as f:
            json.dump(results, f)
def main():
    ...
    parser.add_argument("--recall_tresh")
    parser.add_argument("--db")
    parser.add_argument("--combines")
#
    args = parser.parse_args()
    # for i_id in range(0,1):
    #     for x_id in range(1,2):
    #         print(i_id, x_id)
    run_merge(str(args.mode), str(args.name)+"_"+str(args.i), float(args.recall_tresh), str(args.db), str(args.combines))
